...
export default function CaseDetailManage({
  ...
  return (
    <section className="text-gray-800">
      ...
      <If condition={user?.id === currCase.assignedTo.id}>
        <Then>
          <div className="mt-8">
        ...
        <Else>
          <InfoAlert
            message="Case could only be managed by the case handler"
            className="mt-4"
          />
        </Else>
      </If>
      ...
      {getCurrentStatus() === STATUS.CLOSED && (
        <SuccessAlert
          className="mt-4"
          message={`Case is closed, view the resolution in <b>Resolution</b> tab`}
        />
      )}
    </section>
  );
