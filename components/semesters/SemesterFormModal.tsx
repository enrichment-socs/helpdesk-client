...
export default function SemesterFormModal({ isOpen, setIsOpen, semester }: Props) {
  ...
  const onSubmit: SubmitHandler<FormData> = async (payload) => {
    ...
    await toast.promise(
      semester
        ...
        : SemestersService.addSemester(payload as CreateSemesterDto, user.accessToken),
      {
        ...
        success: (result) => {
          ...
          setIsOpen(false);
          return semester ? `Succesfully updated the semester` : `Successfully added new semester`;
        },
        error: (e) => e.toString(),
      },
    );
                ...
                <form onSubmit={handleSubmit(onSubmit)} className='mt-2 space-y-2'>
                  <div>
                    ...
                    {errors.type?.type === 'required' && (
                      <small className='text-red-500'>Type must be selected</small>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {errors.startYear?.type === 'required' && (
                      <small className='text-red-500'>Start year must be filled</small>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {errors.endYear?.type === 'required' && (
                      <small className='text-red-500'>End year must be filled</small>
                    )}
                  </div>
